...
func SetPlatformEventChannelResourceData(d *schema.ResourceData, m *models.PlatformEventChannel, isDataResource bool) {
...
// Function to perform the following actions:
func PlatformEventChannelModelFromMap(m map[string]interface{}) *models.PlatformEventChannel {
...
func PlatformEventChannelModelFromArrayOfMap(m []interface{}) []*models.PlatformEventChannel {
	mapped := make([]*models.PlatformEventChannel, len(m))
	for i, v := range m {
		mapped[i] = PlatformEventChannelModelFromMap(v.(map[string]interface{}))
	}
	return mapped
}
